...
$50 スタートを前提に、設定・監視・スケールアップの具体的な手順をまとめる。

---

### B-1. スタート時の推奨 `config.json` 設定値

`config.example.json` をコピーして `config.json` を作成し（`cp config.example.json config.json`）、以下の値に書き換える:
...
| 項目 | デフォルト | $50 向け | 意図 |
|---|---|---|---|
| `balance` | 10000 | **50** | 実際の残高に合わせる |
| `max_bet` | 20 | **8** | 残高急増時の暴走防止（現時点では Kelly が先に効く） |
| `kelly_fraction` | 0.25 | **0.20** | 分散確保、ドローダウン抑制 |
| `min_ev` | 0.10 | **0.12** | EV の低い市場を間引き、1件あたりの質を上げる |
| `max_price` | 0.45 | **0.40** | 高値掴み防止（小残高では取り戻しにくい） |
| `min_hours` | 2.0 | **4.0** | 予報の精度が固まる時間を確保 |
| `daily_loss_limit_pct` | 0.10 | **0.15** | $5 → $7.5 に。2〜3 連敗で即停止を緩和 |
| `max_open_positions` | 10 | **5** | 同時保有を絞り、1件あたりのサイズを相対的に大きくする |

> **`state.json` の `starting_balance` は変更しない**。`state.json` は初回起動時に自動生成される。  
> 再起動後に残高を変えたい場合は `data/state.json` を手動で編集する。

---

### B-2. 成長フェーズと設定調整タイミング

残高が増えるにつれて `max_bet` と `max_open_positions` を段階的に引き上げる。他の項目はほぼ据え置きでよい。

| フェーズ | 残高の目安 | `max_bet` | `max_open_positions` | 調整のタイミング |
|---|---|---|---|---|
| Phase 1（スタート） | $30〜$80 | $8 | 5 | — |
| Phase 2（初期成長） | $80〜$200 | $12 | 7 | $80 を超えたら |
| Phase 3（安定運用） | $200〜$500 | $16 | 8 | $200 を超えたら |
| Phase 4（標準設定へ） | $500 以上 | $20 | 10 | $500 を超えたら |

**調整の判断基準**:
- 残高が次フェーズに到達した直後に変える（連勝直後の興奮状態では変えない）
- `daily_loss_limit_pct` は 0.10〜0.15 の範囲で固定し続ける
- `kelly_fraction` は Phase 3 以降に 0.25 へ戻すことを検討

---

### B-3. 実際のベット額の変化（`kelly_fraction=0.20` の場合）

残高別のベット額上限（EV の高い典型的な市場での目安）:

| 残高 | Kelly 上限（5〜6%） | `max_bet` 上限 | 実際のベット額 |
|---|---|---|---|
| $50 | $2.5〜$3.0 | $8 | **$2.5〜$3.0**（Kelly が効く） |
| $100 | $5.0〜$6.0 | $8 | **$5.0〜$6.0**（Kelly が効く） |
| $150 | $7.5〜$9.0 | $8 | **$8**（max_bet が効く） |
| $200 | — | $12（フェーズ引上げ後） | **$10〜$12**（Kelly が効く） |
| $400 | — | $16 | **$14〜$16**（Kelly が効く） |
| $500 | — | $20 | **$18〜$20**（Kelly が効く） |

残高が増えると Kelly が産出するベット額も増えるが、`max_bet` で頭打ちになる。`max_bet` を段階的に引き上げることで、残高比例のリスク管理を維持しながら成長できる。

---

### B-4. キャリブレーションが効くまでの期間

キャリブレーション（`calibration_min=30`）が発動するには解決済みマーケット 30 件が必要。

| 週あたりトレード数 | 30 件到達までの期間 | その間の sigma |
|---|---|---|
| 3〜5 件 | 6〜10 週（1.5〜2.5 ヶ月） | デフォルト sigma（2.0°F / 1.2°C）を使用 |
| 7〜10 件 | 3〜4 週 | 同上 |

**キャリブレーション前の注意点**:
- デフォルト sigma が実態より大きい場合 → 確率を保守的に計算 → エントリーが減る（安全側）
- デフォルト sigma が実態より小さい場合 → EV を過大評価 → 過信してエントリー（危険側）
- `vc_key` を最初から設定しておかないと、キャリブレーションに必要な `actual_temp` が溜まらない

---

### B-5. 何を見れば「うまくいっているか」わかるか

短期（1〜2 週間）の勝敗は確率的なノイズが大きく、戦略の優劣を判断できない。以下の指標を見る。
...
確認ポイント:
- `EV` が一貫して `0.12` 以上か（フィルタが機能しているか）
- `close_reason` の内訳: `take_profit` が多いか、`stop_loss` ばかりでないか
- `forecast_changed` によるクローズが多い場合は予報精度の問題（sigma を上げる方向を検討）
...
| パネル | 何を見るか |
|---|---|
| Balance History | 右肩上がりのトレンドがあるか。急落がないか |
| Daily PnL | 特定の曜日・季節に偏りがないか |
| City Stats | 特定都市だけ大きく負けていないか（予報ソースの問題の可能性） |
| Bot Log | `WARNING` / `ERROR` の頻度 |
...
|---|---|
| 「利益が出ているか」 | 最低 30〜50 件 |
| 「勝率が確率と整合しているか」 | 50〜100 件 |
| 「設定変更の効果検証」 | 変更前後それぞれ 20 件以上 |
...
| パターン | 原因 | 対処 |
|---|---|---|
| 毎日 `daily_loss_limit` が発動する | 市場の値動きが荒い / `min_ev` が低すぎる | `daily_loss_limit_pct` を 0.20 に緩和 + `min_ev` を 0.15 に上げる |
| エントリーが全く発生しない | `min_ev` が高すぎる / `max_price` が低すぎる | `min_ev` を 0.10 に戻し、`min_volume` を下げて様子を見る |
| 短期間で残高が半分以下になる | Kelly 計算が合っていない / sigma が小さすぎる | `kelly_fraction` を 0.15 に下げ、`sigma_f` を 2.5 に上げてみる |
| `forecast_changed` クローズが多発 | 予報ソースの精度が低い日が続いている | ログで ECMWF / HRRR 両方が失敗していないか確認 |
| 残高が増えてきたのにベットが小さいまま | `max_bet` を更新していない | B-2 の成長フェーズ表に従って `max_bet` を引き上げる |

---

### B-7. 最初の 1 週間でやること

1. **`vc_key` を取得して設定する**（Visual Crossing 無料枠で可）  
   → `actual_temp` が溜まらないとキャリブレーションが始まらない

2. **`python weatherbet.py run` を起動し、初回スキャンのログを確認する**  
   → `[BUY]` が 0 件の場合はフィルタ条件の見直しが必要

3. **ダッシュボードを確認する（`http://localhost:8000/sim_dashboard_repost.html`）**  
   → Open Positions に何かが入ったら、価格と EV を確認する

4. **最初の 1〜2 件の結果だけで設定を変えない**  
   → 2 連敗は確率上ありえる。30 件貯まるまでは設定を維持する

5. **City Stats が埋まり始めたら、勝率の低い都市を確認する**  
   → その都市の予報精度が低い可能性がある（`calibration.json` の sigma が参考になる）
