...
3. [ベースボット（bot_v1.py）](#3-ベースボット-bot_v1py)
4. [フルボット（bot_v2.py）](#4-フルボット-bot_v2py)
...
> **前提**: `bot_v2.py` を一定期間動かして `data/markets/` にデータが溜まってから使う。
...
フォワードテストを有効に使うには、`vc_key` を設定して `bot_v2.py` を動かし続けること。クローズした市場の `actual_temp` が自動取得されるたびにフォワードテストの母集団が増える。

### データ蓄積の仕組みと制約

`bot_v2.py` がスキャン → ポジション取得 → 翌日解決確認、という流れで `data/markets/*.json` を自動更新する。**特別なデータ収集操作は不要。ボットを動かし続けるだけで蓄積される。**
...
**フォワードテストとの使い分け**: `--forward` フラグを使うと、ポジションを取っていない市場（スキップした市場）も含めて全件評価できる。スキップした市場が実際に勝ちだったかどうかを確認でき、`min_ev` などのパラメータ最適化に有効。`bot_v2.py` は vc_key がある場合、クローズした全市場の `actual_temp` を自動取得する。
...
### `simulation.json`（bot_v1.py 専用、ルートに生成）
...
`python bot_v2.py dashboard` で `data/dashboard.json` の生成と HTML オープンは実装済み。  
HTML 側の fetch 連携は追加実装余地がある。

利用コマンド:
```bash
python bot_v2.py dashboard    # data/dashboard.json を生成してブラウザで開く
```
...
現在 `bot_v2.py` は設定・API・数学・状態管理・CLI が1ファイルに混在（約1050行）。
...
この表記ズレは修正済み。現在のエントリポイント表記は `python bot_v2.py` に統一されている。
