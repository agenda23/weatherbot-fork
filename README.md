...
| ダッシュボード | `run` 起動時に HTTP サーバー自動起動。残高チャート・都市別成績・日次 PnL・ログを 30 秒ポーリングで表示 |
...
| ダッシュボード | `dashboard_port` | HTTP サーバーポート（デフォルト: 8000） |
...
python weatherbet.py              # ボット起動（毎時スキャン + 10分監視）
                                  # → http://localhost:8000/sim_dashboard_repost.html を自動起動
python weatherbet.py status       # 残高・オープンポジション表示
python weatherbet.py report       # 全解決マーケットの詳細レポート
python weatherbet.py dashboard    # data/dashboard.json を 1 回生成してブラウザで開く
```
...
| `data/dashboard.json` | ダッシュボード配信データ（毎時スキャン・10 分監視ごとに自動更新） |
| `data/balance_history.json` | 残高時系列（最大 500 件、ダッシュボードチャート用） |
...
│       ├── scanner.py           # 毎時スキャン
│       ├── monitor.py           # 10 分監視
│       ├── report.py            # ステータス・ダッシュボード JSON 生成
│       ├── calibration.py       # sigma 更新
│       ├── notify.py            # ログ・Discord・API 障害追跡
│       ├── clob.py              # CLOB クライアント・署名
│       ├── forecast/            # ecmwf / hrrr / metar / blend
│       ├── market/              # polymarket / parser
│       ├── strategy/            # probability / kelly / risk
│       └── storage/             # state / markets
├── tests/
│   └── test_core_functions.py   # pytest テスト（44 テスト）
├── docs/
│   ├── TODO.md                  # 開発ロードマップ
│   ├── IMPLEMENTATION_PLAN.md   # 実装計画・フェーズ管理
│   ├── DASHBOARD_PLAN.md        # ダッシュボード実装計画
│   ├── MANUAL.md                # 運用マニュアル
...
| Phase 1 | テスト導入・構造化ログ・モジュール分割（`src/weatherbet/`） | ✅ 完了 |
| Phase 2 | キャリブレーション RMSE 化 | ✅ 完了 |
| Phase 3 | 日次損失制限・相関ガード・動的ストップ・連続テイクプロフィット | ✅ 完了 |
| Phase 5 | リアルタイムダッシュボード（HTTP サーバー自動起動・30 秒ポーリング） | ✅ 完了 |
