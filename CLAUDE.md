...
```bash
# v2 (full bot — use this for development)
python bot_v2.py           # main scan loop (hourly)
python bot_v2.py status    # balance and open positions
python bot_v2.py report    # full resolved-market breakdown

# v1 (simple baseline, US-only)
python bot_v1.py           # paper scan
python bot_v1.py --live    # simulate trades against virtual balance
python bot_v1.py --positions
python bot_v1.py --reset
```

**Install deps**: `pip install requests` (only external dependency)

## Important naming discrepancy

README and the `bot_v2.py` docstring refer to `weatherbet.py`, but that file does not exist. The full-featured bot is `bot_v2.py`. When the README says `python weatherbet.py`, read it as `python bot_v2.py`.
...
|------|--------|-----------------|------------|-----------|
| `bot_v1.py` | 6 US | NWS gridpoint + METAR obs | Flat 5% position size | `simulation.json` (root) |
| `bot_v2.py` | 20 (US/EU/Asia/CA/SA/OC) | ECMWF + HRRR (Open-Meteo) + METAR | Kelly criterion + EV filter + stops | `data/state.json`, `data/markets/`, `data/calibration.json` |
...
7. **Calibration** — after resolution, `run_calibration()` computes MAE per city×source from `forecast_snapshots[n].best` vs `actual_temp`, updates sigma in `data/calibration.json`. Requires `calibration_min` (default 30) resolved samples.
...
## config.json keys (v2 schema — current)
...
**v1 uses different keys**: `entry_threshold`, `exit_threshold`, `max_trades_per_run`, `min_hours_to_resolution`, `locations`. The current `config.json` in the repo is v2-format; v1 falls back to hardcoded defaults when its keys are missing.
...
## Data directory layout (v2, created at runtime)
...
`simulation.json` at repo root is v1-only.
